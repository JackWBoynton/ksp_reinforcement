...
class ActorCritic:
	def __init__(self, env, sess):
		...
		self.actor_grads = tf.gradients(ys=self.actor_model.output, 
			xs=actor_model_weights, grad_ys=-self.actor_critic_grad) # dC/dA (from actor)
		...
		self.critic_grads = tf.gradients(ys=self.critic_model.output, 
			xs=self.critic_action_input) # where we calcaulte de/dC for feeding above
	...
	def _train_critic(self, samples):
		for sample in samples:
			...
			#print([cur_state.flatten(), action.flatten()], reward, cur_state.shape, action.shape)
